...
export function parseCommandArgs(args: string): SimplebenchOptions {
  ...
  return { model: tokens.find(token => !token.startsWith("--")), allModels: tokens.includes("--all"), writeArtifact: !tokens.includes("--no-artifact") };
}
  ...
  handler: async (args, ctx) => {
    ...
    if (arg === "--help") {
      ctx.ui.notify(
        "🔍 Simplebench Extension\n\n" +
        "📋 Usage:\n" +
        "  /simplebench [model] [--no-artifact] - Test current or specific model\n" +
...
pi.registerTool({
  ...
  execute: async (_toolCallId, _params, _signal, _onUpdate, ctx) => {
    ...
    const options: SimplebenchOptions = { model: params?.model as string | undefined, allModels: false, writeArtifact: params?.no_artifact !== true };
