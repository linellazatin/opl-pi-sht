 ...
 *
 * Entirely model-invoked: the LLM calls this tool instead of listing questions
 * in plain text. There is no slash command or keybinding.
 */
...
export default function questionnaire(pi: ExtensionAPI) {
	pi.registerTool({
		...
		label: "Questionnaire",
		description:
			"Ask the user one or more questions. Use for clarifying requirements, getting preferences, or confirming decisions. For single questions, shows a simple option list. For multiple questions, shows a tab-based interface. If this tool errors (e.g. non-interactive session), fall back to asking the questions in plain text.",
		promptSnippet: "Ask the user clarifying questions with selectable options",
		promptGuidelines: [
			"Use questionnaire when you need to ask the user one or more clarifying questions with selectable options, instead of listing questions in plain text and waiting for the user to answer by number.",
			"Prefer questionnaire whenever a decision has a small set of discrete choices (scope, approach, yes/no confirmation); it lets the user pick options directly and returns structured answers.",
		],
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const result = await ctx.ui.custom<QuestionnaireResult>((tui, theme, _kb, done) => {
				...
				/** Select the option at the given index — enters input mode for "other", else saves + advances. */
				function selectOption(q: Question, opts: RenderOption[], index: number) {
				...
				function handleInput(data: string) {
					...
					// Digit-key direct selection: 1-9 select that numbered option, 0 selects the 10th.
					if (q && data.length === 1 && data >= "0" && data <= "9") {
						const n = data === "0" ? 10 : Number(data);
						const target = n - 1;
						if (target < opts.length) {
							optionIndex = target;
							selectOption(q, opts, target);
						}
						return;
					}
					...
					if (matchesKey(data, Key.enter) && q) {
						selectOption(q, opts, optionIndex);
						return;
					}
