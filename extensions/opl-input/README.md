...
User config lives in `~/.pi/agent/configs/opl-input.json`. Create it to override defaults:
...
Chat mode gets its own border/prefix styling like plan mode — see the [`opl-modes`](../opl-modes/README.md) extension. Precedence when multiple modes are active: bash > plan > chat > default (in practice modes are mutually exclusive).
