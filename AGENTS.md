...
A private Pi coding-agent extension kit packaged as a Git-installable Pi package. Extension directories and configuration files use the `opl-` prefix. Preserve established Pi-facing commands and compatibility names, including `/init`, `/chat`, `/plan`, `/execute`, `/todos`, `todo`, and `questionnaire`.
