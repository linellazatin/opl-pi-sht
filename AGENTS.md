...
`opl-browser` provides one lazy Playwright browser dispatcher plus structured extraction of rendered pages. `opl-webaccess` provides provider-backed search and readable URL/PDF extraction. `opl-ctxtrim` trims only known context-mode `ctx_*` schema descriptions, preserves all schema fields, and fails open for unknown payloads. `opl-init` writes a deterministic fingerprinted `AGENTS.md` straight from the repository crawl, overwriting a stale guide outright; plain `/init` injects no prompt, and `/init --refine` is the opt-in single model request. `AGENTS.md` itself is excluded from the fingerprint. `opl-simplebench` provides provider-aware benchmarking, coding-lite tasks, deterministic grounded research (`--test-all`) plus optional live-search smoke test (`--research-live`), metrics, scoring, direct llama-server metadata, llamagputop stats, tagged artifacts, and config-templated named multi-run sequences (`--sequence[=<name>]`). `opl-todo` stores branch-aware tasks; `opl-questionnaire` provides interactive choices.
